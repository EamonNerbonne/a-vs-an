...
namespace AvsAnLib {
	public static class AvsAn {
		...
		struct Node : IComparable<Node> {
			...
			public void Add(string prefix, int depth, Ratio prefixRatio) {
				if (prefix.Length == depth) {
					...
					while (true) {
						if (i < kids)
							if (SortedKids[i].c == prefix[depth])
								break;
							else
								i++;
						else {
							if (kids == SortedKids.Length)
								Array.Resize(ref SortedKids, kids * 2);
							SortedKids[kids++].c = prefix[depth];
						}
					}
					SortedKids[i].Add(prefix, depth + 1, prefixRatio);
				}
			}
			...
			public override string ToString() {
				return c + " " + SortedKids.Length + " " + ratio.aCount + "/" + ratio.anCount;
			}
		...
		static AvsAn() {
			...
			var plainBuild = sw.Elapsed;
			root.Minimize();
			var plainBuild2 = sw.Elapsed;
			Console.WriteLine("Building took " + plainBuild.TotalMilliseconds + "; " + (plainBuild2 - plainBuild).TotalMilliseconds);
		}
